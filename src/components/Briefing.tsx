  ...
  return (
    <div className="ck-brief">
      <div className="ck-board-title" style={{ marginBottom: 8 }}>
        <b>Daily briefing</b>
        <span>{leads.length ? `to ${leads.length} lead${leads.length === 1 ? '' : 's'}` : 'no leads hired yet'}</span>
      ...
      <div className="ck-composer" style={{ marginTop: 0 }}>
        <textarea value={text} onChange={(e) => setText(e.target.value)} rows={compact ? 2 : 3}
          placeholder={leads.length ? 'What matters today. Every lead reads it, takes what is theirs, hands over the rest.' : 'Hire a department on the Team page, then brief the leads here each morning.'}
          onKeyDown={(e) => { if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) { e.preventDefault(); void send() } }} />
        <div className="ck-composer-row">
          <span style={{ fontSize: 11.5, color: 'var(--ck-faint)' }}>Also from your phone: /brief in Telegram.</span>
          <button className="ck-go" disabled={busy || !text.trim() || !leads.length} onClick={() => void send()}>{busy ? 'Sending…' : 'Brief the leads'}</button>
        </div>
      </div>
