...
const PRIMARY_ORDER = ['dashboard', 'create', 'roles', 'clients']

const bar: React.CSSProperties = {
  position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 40,
  display: 'flex', background: 'var(--ck-ink)', borderTop: '1px solid rgba(244,240,231,0.12)',
  paddingBottom: 'env(safe-area-inset-bottom)',
}
const tab = (active: boolean): React.CSSProperties => ({
  flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3,
  padding: '9px 4px 8px', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'none',
  color: active ? 'var(--ck-accent)' : 'var(--text-on-ink-muted)',
})
    ...
    <>
      <nav style={bar}>
        {primary.map((item) => (
          <NavLink key={item.key} to={item.path} end={item.path === '/'} style={({ isActive }) => tab(isActive)}>
            {({ isActive }) => (<><span style={{ ...glyphS, color: isActive ? 'var(--ck-accent)' : 'var(--text-on-ink-faint)' }}>{item.glyph}</span><span style={labelS}>{item.label.split(' ')[0]}</span></>)}
          </NavLink>
        ))}
      ...
      {more && (
        <div onClick={() => setMore(false)} style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'flex-end' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: 'var(--ck-ink)', color: 'var(--ck-bg)', borderRadius: '18px 18px 0 0', padding: '10px 12px calc(16px + env(safe-area-inset-bottom))', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            {rest.map((item) => (
              <NavLink key={item.key} to={item.path} end={item.path === '/'} onClick={() => setMore(false)}
                style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 12px', borderRadius: 12, textDecoration: 'none', color: 'var(--ck-bg)', fontSize: 15 }}>
            ...
            <button onClick={() => { setMore(false); openSelector() }} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 12px', width: '100%', background: 'none', border: 'none', color: 'var(--ck-bg)', fontSize: 15, cursor: 'pointer', textAlign: 'left' }}>
