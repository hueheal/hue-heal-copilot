...
/* Client document editor: one immersive branded document per client, drafted by
   the copilot from the client's context. Content docs are block documents; form
   docs are step-by-step questionnaires the client completes in their space. */
...
export default function ClientDocEditor() {
  ...
  useEffect(() => {
    ...
    getClientDoc(docId).then((d) => {
      ...
      setBlocks(Array.isArray(d.blocks) ? (d.blocks as unknown as Block[]) : [])
  ...
  /* ---- AI drafting: customised to this client, in the brand voice ---- */
  async function generate() {
    ...
    try {
      const res = await fetch(`${functionsBase}/generate-client-doc`, {
        ...
        body: JSON.stringify({
          kind: doc.kind, kindLabel: kind.label, isForm,
      ...
      if (isForm && Array.isArray(r.steps)) {
  ...
  async function save() {
    ...
    try {
      await updateClientDoc(doc.id, { title, dek, blocks: blocks as unknown[], form: steps as unknown[], shared })
      setStatus('Saved')
  ...
  if (gated) {
    return <EditorShell ctype="Document" subline="Sign in to edit" onDone={() => {}} doneDisabled rail={<p style={{ fontSize: 14, color: 'var(--text-muted)' }}>Sign in (bottom-left) to edit.</p>} canvas={<div />} />
  }
  if (!doc) {
    return <EditorShell ctype="Document" subline="Loading…" onDone={() => {}} doneDisabled rail={<p style={{ fontSize: 14, color: 'var(--text-faint)' }}>{status ?? 'Loading…'}</p>} canvas={<div />} />
  }
  ...
  /* ---- canvas: the immersive branded document ---- */
  const cover = (
    <div style={{ background: 'var(--hh-anthracite)', color: '#F4F0E7', borderRadius: '14px 14px 0 0', padding: isMobile ? '38px 26px' : '54px 56px' }}>
      ...
      rail={
        <div>
          <div style={{ border: '1px solid var(--hh-line)', borderRadius: 14, padding: 16, background: 'var(--hh-bone)' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 6 }}>Customised to {client?.name ?? 'the client'}, in {brand?.name ?? 'the brand'}’s voice.</div>
          </div>
