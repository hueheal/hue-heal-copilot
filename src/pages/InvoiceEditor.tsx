...
export default function InvoiceEditor() {
  ...
  if (!inv) return <div style={{ padding: 40, color: 'var(--ck-muted)' }}>{status ?? 'Loading invoice…'}</div>
  ...
  return (
    <div>
      <div className="no-print" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '20px 40px', borderBottom: '1px solid var(--ck-line)' }}>
        <button onClick={() => nav(inv?.client_id ? `/clients/${inv.client_id}` : '/clients')} className="hh-btn" style={{ background: 'none', border: 'none', color: 'var(--ck-accent)', fontSize: 13 }}>⟵ {inv?.client_name || 'Clients'}</button>
        <div style={{ fontSize: 13, color: 'var(--ck-faint)' }}>{inv.client_name} · {inv.status}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {items.map((li, i) => (
                <div key={i} style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => removeItem(i)} className="hh-btn" style={{ background: 'none', border: 'none', color: 'var(--ck-faint)', fontSize: 16 }}>×</button>
                </div>
              ))}
              <button onClick={addItem} className="hh-btn" style={{ background: 'none', border: '1px dashed var(--hh-line)', borderRadius: 10, padding: '9px', color: 'var(--ck-muted)', fontSize: 13 }}>＋ Add line item</button>
            </div>
          </div>
