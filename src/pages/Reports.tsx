  ...
  return (
    <>
      ...
      <div style={{ padding: '30px 40px' }}>
        ...
        <div
          className="hh-atmos"
          style={{ borderRadius: 18, padding: '36px 32px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, marginBottom: 16, color: 'var(--ck-bg)' }}
        >
          {KPIS.map((k) => (
            <div key={k.label}>
              <div className="hh-serif" style={{ fontWeight: 300, fontSize: 64, lineHeight: 1, color: 'var(--ck-accent)' }}>
                {k.value}
              </div>
        ...
        <div style={{ background: 'var(--ck-surface)', border: '1px solid var(--ck-line-strong)', borderRadius: 16, padding: 24 }}>
          <div style={{ fontSize: 12, letterSpacing: '0.04em', color: 'var(--ck-faint)', marginBottom: 18 }}>
            Reach by channel · last 30 days
          </div>
          {CHANNELS.map((c) => (
            <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 0', borderTop: '1px solid var(--ck-line)' }}>
              <div style={{ width: 120, fontSize: 14, fontWeight: 500 }}>{c.name}</div>
              <div style={{ flex: 1, height: 8, background: 'var(--ck-bg)', borderRadius: 999, overflow: 'hidden' }}>
                <div style={{ width: `${c.bar * 100}%`, height: '100%', background: 'var(--ck-accent)', borderRadius: 999 }} />
              </div>
