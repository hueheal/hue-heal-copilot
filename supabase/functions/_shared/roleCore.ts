      ...
      images: {
        ...
        items: {
          ...
          properties: {
            ...
            surface: { type: 'string', description: 'A surface key from IMAGES, exactly as listed (e.g. "hero-card"), nothing else in the field.' },
            shot: { type: 'string', description: 'Optional shot type from IMAGES. Leave out to take the category\'s default.' },
          },
...
export const DESK_TASK =
  'Write the founder\'s desk from the replies your colleagues have sent back (in YOUR TEAM\'S CONTRIBUTIONS) and the briefing they answered, ordered by THE FOUNDER\'S PRIORITIES in the org brief: the founder\'s own list outranks anything a department wants. Sections, in this order and with these headings: "Now" (the single thing the founder should do or decide first, which priority number it serves, and why it unblocks the most), "Next" (at most three, one line each, each tagged with its priority number), "Parked" (everything else, one line each with why it can wait), "Decisions waiting on you" (each with a proposed default so the founder can answer in a word). If a reply proposes work that serves no priority, park it and say so. Under 300 words. Do not repeat the replies; compress them. Propose no content pieces, raise no needs, no experiments.'
