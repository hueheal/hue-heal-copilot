...
Deno.serve(async (req) => {
  ...
  const tool = isForm ? FORM_TOOL : CONTENT_TOOL
  let resp: Response
  try {
  ...
  const result = use?.input as { title?: string; sections?: unknown[]; steps?: unknown[] } | undefined
  if (!result || (isForm ? !Array.isArray(result.steps) || !result.steps.length : !Array.isArray(result.sections) || !result.sections.length)) {
    return json({ error: 'No draft returned' }, 502)
  }
  return json({ result })
})
